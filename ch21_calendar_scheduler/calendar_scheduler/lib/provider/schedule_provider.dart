...
class ScheduleProvider extends ChangeNotifier {
  ...
  void getSchedules({
    ...
    final resp = await scheduleRepository.getSchedules(
        date: date, accessToken: accessToken!); // GET 메서드 보내기
  ...
  void createSchedule({
    ...
    try {
      final savedSchedule = await scheduleRepository.createSchedule(
          schedule: schedule, accessToken: accessToken!); // API 요청을 합니다.
  ...
  void deleteSchedule({
    ...
    try {
      await scheduleRepository.deleteSchedule(
          id: id, accessToken: accessToken!); // ➊ 삭제함수 실행
    } catch (e) {
      cache.update(
        // ➋ 삭제 실패시 캐시 롤백하기
        date,
  ...
  Future<void> register({
    ...
    updateTokens(
      refreshToken: resp.refreshTocken,
  ...
  Future<void> login({
    ...
    final _ = await authRepository.login(
      email: email,
      password: password,
    );

    updateTokens(refreshToken: refreshToken, accessToken: accessToken);
  }
  ...
  rotateToken(
      ...
      // true => refreshToken 재발급
      // false => accessToken 재발급
